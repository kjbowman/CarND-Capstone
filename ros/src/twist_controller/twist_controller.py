...
class Controller(object):
    def __init__(self, vehicle_mass, fuel_capacity, brake_deadband,
        ...
        mn = 0.0    # min throttle
        mx = 0.2    # max throttle
        self.thottle_controller = PID(kp, ki, kd, mn, mx)

        tau = 0.5   # 1/(2pi*tau) = f_cutoff
        ts = 0.02   # t_sample
    ...
    def control(self, current_vel, dbw_enabled, linear_vel, angular_vel):
        ...
        steering = self.yaw_controller.get_sterring(linear_vel, angular_vel, current_vel)

        vel_error = linear_vel = current_vel
        self.last_vel = current_vel
        
        current_time = rospy.get_time()
        sample_time = current_time - self.last_timeself.last_time = current_time
